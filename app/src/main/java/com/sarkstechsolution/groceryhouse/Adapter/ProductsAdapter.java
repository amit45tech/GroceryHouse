...
public class ProductsAdapter extends FirebaseRecyclerAdapter<Products, ProductsAdapter.ProductViewHolder> {
    ...
    @Override
    protected void onBindViewHolder(@NonNull ProductsAdapter.ProductViewHolder holder, int position, @NonNull Products model) {

        float mrp = Float.parseFloat(model.getMrp().toString());
        float sp = Float.parseFloat(model.getSp().toString());

        int discount =Math.round(((mrp-sp)/mrp)*100);
